...
var eddsaPoseidon = (function (exports) {
  ...
  */
  ...
  function beBufferToBigInt(value) {
      ...
      return BigInt(`0x${Buffer$1.from(value).toString("hex")}`);
  }
  /**
  ...
  function leBufferToBigInt(value) {
      requireTypes(value, "value", ["Buffer", "Uint8Array"]);
      return BigInt(`0x${Buffer$1.from(value).reverse().toString("hex")}`);
  }
  /**
  ...
  function leBigIntToBuffer(value, size) {
      ...
      const buffer = Buffer$1.alloc(size, 0);
      const fromHex = Buffer$1.from(hex, "hex").reverse();
  ...
  */
   ...
   */
  const zo = Buffer$1.from([0x01]);
  const oo = Buffer$1.from([0x81]);
  ...
  getDefaultExportFromCjs(util$2);
  ...
  getDefaultExportFromCjs(blake2b_1);
  ...
  getDefaultExportFromCjs(blake2s_1);
  ...
  getDefaultExportFromCjs(blakejs);
  ...
  class Blake2b {
      ...
      constructor(outlen = 64, key) {
          ...
          if (outlen <= 0 || outlen > 64)
              ...
              this.outlen = outlen;
          this.context = blakejs.blake2bInit(this.outlen, key);
      }
      /**
       * Updates the hash with new data. This method can be called multiple
       * times to incrementally add data to the hash computation.
       ...
       */
      update(input) {
          blakejs.blake2bUpdate(this.context, input);
          return this;
      }
      /**
       * Completes the hash computation and returns the final hash value.
       ...
       */
      digest() {
          return Buffer.from(blakejs.blake2bFinal(this.context));
      }
  }
  ...
  (function (SupportedHashingAlgorithms) {
      SupportedHashingAlgorithms["BLAKE1"] = "blake-1";
      SupportedHashingAlgorithms["BLAKE2b"] = "blake-2b";
  })(SupportedHashingAlgorithms || (SupportedHashingAlgorithms = {}));
  const EdDSAPoseidonFactory = (algorithm) => {
      /**
      ...
      function signMessage(privateKey, message) {
          ...
          const rBuff = hashInput(Buffer$1.concat([hash.slice(32, 64), msgBuff]), algorithm);
  ...
  const { EdDSAPoseidon, derivePublicKey, deriveSecretScalar, packPublicKey, packSignature, signMessage, unpackPublicKey, unpackSignature, verifySignature } = EdDSAPoseidonFactory(SupportedHashingAlgorithms.BLAKE1);
