...
const EdDSAPoseidonFactory = (algorithm) => {
    ...
    const deriveSecretScalar = (privateKey) => {
        ...
        return scalar__namespace.shiftRight(conversions.leBufferToBigInt(hash), BigInt(3)) % babyJubjub.subOrder;
    };
    /**
     * Derives a public key from a given private key using the
     * {@link https://eips.ethereum.org/EIPS/eip-2494|Baby Jubjub} elliptic curve.
    ...
    function signMessage(privateKey, message) {
        ...
        const rBuff = hashInput(buffer.Buffer.concat([hash.slice(32, 64), msgBuff]), algorithm);
...
const { EdDSAPoseidon, derivePublicKey, deriveSecretScalar, packPublicKey, packSignature, signMessage, unpackPublicKey, unpackSignature, verifySignature } = EdDSAPoseidonFactory(SupportedHashingAlgorithms.BLAKE2b);
